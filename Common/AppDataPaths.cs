...
{
    /// <summary>
    /// Resolves the application's user-local data paths for reports, logs, config, review checklist, update checks, and IL cache.
    /// Tests can override the LocalApplicationData root via <see cref="AppContext.SetData(string, object?)"/>.
    /// レポート、ログ、設定、レビューチェックリスト、更新確認、IL キャッシュ向けのユーザーローカルデータパスを解決します。
    ...
    internal static class AppDataPaths
    {
        ...
        private const string UPDATE_CHECK_CACHE_FILE_NAME = "update-check.json";
        ...
        /// <summary>Gets the update-check cache file path. / 更新確認キャッシュファイルのパスを返します。</summary>
        internal static string GetUpdateCheckCacheFileAbsolutePath()
            => Path.Combine(GetApplicationDataRootAbsolutePath(), UPDATE_CHECK_CACHE_FILE_NAME);
