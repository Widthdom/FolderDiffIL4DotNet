...
- **SHA256 integrity verification for reviewed HTML** — The "Download as reviewed" workflow now computes a SHA256 hash of the reviewed HTML using the Web Crypto API and downloads a companion `.sha256` verification file (e.g. `diff_report_20260315_reviewed.html.sha256`) alongside the reviewed HTML. The hash is embedded inside the reviewed HTML itself via a placeholder approach (avoids the chicken-and-egg problem), enabling in-browser self-verification: the reviewed HTML's header includes a "Verify integrity" button that prompts the user to select the file, recomputes the hash, and displays a pass/fail dialog. The `.sha256` file follows the `sha256sum`/`shasum` format (`<hash>  <filename>`) and can be verified on any OS (Linux: `sha256sum -c`, macOS: `shasum -a 256 -c`, Windows: `Get-FileHash` in PowerShell). Submitting the reviewed HTML together with the `.sha256` file constitutes a tamper-proof audit record. This closes the integrity verification gap where the original `diff_report.html` hash in `audit_log.json` does not cover the reviewed HTML (which differs due to embedded review state and read-only banner). Added 1 test in [`HtmlReportGenerateServiceTests`](FolderDiffIL4DotNet.Tests/Services/HtmlReportGenerateServiceTests.cs) verifying SHA256 integrity code presence (`crypto.subtle.digest`, `__reviewedSha256__` sentinel, `verifyIntegrity` function).
...
- **レビュー済み HTML の SHA256 整合性検証** — 「Download as reviewed」ワークフローで、Web Crypto API を用いてレビュー済み HTML の SHA256 ハッシュを計算し、レビュー済み HTML と合わせてコンパニオン `.sha256` 検証ファイル（例: `diff_report_20260315_reviewed.html.sha256`）をダウンロードするよう改善。ハッシュはプレースホルダ方式でレビュー済み HTML 自体にも埋め込まれ（鶏と卵問題を回避）、ブラウザ内での自己検証を実現: レビュー済み HTML のヘッダーに「Verify integrity」ボタンを追加し、クリックするとファイル選択 → ハッシュ再計算 → 合格/不合格ダイアログが表示される。`.sha256` ファイルは `sha256sum`/`shasum` 形式（`<hash>  <filename>`）に従い、全 OS で検証可能（Linux: `sha256sum -c`、macOS: `shasum -a 256 -c`、Windows: PowerShell の `Get-FileHash`）。レビュー済み HTML と `.sha256` ファイルを一緒に提出することで、改ざんのない監査記録となる。`audit_log.json` 内のオリジナル `diff_report.html` ハッシュがレビュー済み HTML（レビュー状態の埋め込みと読み取り専用バナーにより異なる）をカバーしないインテグリティ検証のギャップを解消。[`HtmlReportGenerateServiceTests`](FolderDiffIL4DotNet.Tests/Services/HtmlReportGenerateServiceTests.cs) に SHA256 整合性コード存在確認のテスト 1 件を追加（`crypto.subtle.digest`、`__reviewedSha256__` センチネル、`verifyIntegrity` 関数）。
