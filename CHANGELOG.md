...
- **SHA256 integrity verification for reviewed HTML** — The "Download as reviewed" workflow now computes a SHA256 hash of the reviewed HTML using the Web Crypto API and downloads a companion `.sha256` verification file (e.g. `diff_report_20260315_reviewed.html.sha256`) alongside the reviewed HTML. The hash is also displayed in the save-status area for manual verification. The `.sha256` file follows the `sha256sum` format (`<hash>  <filename>`) and can be verified with `sha256sum -c`. This closes the integrity verification gap where the original `diff_report.html` hash in `audit_log.json` does not cover the reviewed HTML (which differs due to embedded review state and read-only banner). Added 1 test in [`HtmlReportGenerateServiceTests`](FolderDiffIL4DotNet.Tests/Services/HtmlReportGenerateServiceTests.cs) verifying SHA256 integrity code presence.
...
- **レビュー済み HTML の SHA256 整合性検証** — 「Download as reviewed」ワークフローで、Web Crypto API を用いてレビュー済み HTML の SHA256 ハッシュを計算し、レビュー済み HTML と合わせてコンパニオン `.sha256` 検証ファイル（例: `diff_report_20260315_reviewed.html.sha256`）をダウンロードするよう改善。ハッシュは手動検証用に save-status エリアにも表示。`.sha256` ファイルは `sha256sum` 形式（`<hash>  <filename>`）に従い、`sha256sum -c` で検証可能。`audit_log.json` 内のオリジナル `diff_report.html` ハッシュがレビュー済み HTML（レビュー状態の埋め込みと読み取り専用バナーにより異なる）をカバーしないインテグリティ検証のギャップを解消。[`HtmlReportGenerateServiceTests`](FolderDiffIL4DotNet.Tests/Services/HtmlReportGenerateServiceTests.cs) に SHA256 整合性コード存在確認のテスト 1 件を追加。
