...
- **Simplify Verify integrity to `.sha256`-only verification** — Removed the HTML self-verification path and `input.accept` filter from `verifyIntegrity()`. Since the reviewed HTML is "self", only the companion `.sha256` file needs to be selected. The file picker now opens without extension restrictions (macOS Safari ignores unknown extensions like `.sha256` in the `accept` attribute). The guard condition now checks `__finalSha256__` instead of `__reviewedSha256__`. Updated [`diff_report.js`](Services/HtmlReport/diff_report.js) and [`doc/samples/diff_report.html`](doc/samples/diff_report.html).
...
- **Verify integrity を `.sha256` 専用検証に簡素化** — `verifyIntegrity()` から HTML 自己検証パスと `input.accept` フィルタを削除。レビュー済み HTML は「自分自身」であるため、コンパニオン `.sha256` ファイルのみ選択すれば十分です。ファイルピッカーは拡張子制限なしで開きます（macOS Safari は `.sha256` のような未知の拡張子を `accept` 属性で無視するため）。ガード条件を `__reviewedSha256__` から `__finalSha256__` に変更。[`diff_report.js`](Services/HtmlReport/diff_report.js) および [`doc/samples/diff_report.html`](doc/samples/diff_report.html) を更新。
