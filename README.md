...
```
 ...
 │     → diff_report_YYYYMMDD_reviewed.html  (SHA256 hash embedded)│
 │     → diff_report_YYYYMMDD_reviewed.html.sha256                 │
 ├─────────────────────────────────────────────────────────────────┤
 │  5. Verify reviewed HTML (two methods)                          │
 │     A. In-browser: open the reviewed HTML, click                │
 │        "Verify integrity" in the header, select the file        │
 │        → pass/fail dialog                                       │
 │     B. Command line: use the companion .sha256 file             │
...
| Windows (PowerShell) | `if ((Get-FileHash diff_report_20260315_reviewed.html -Algorithm SHA256).Hash -eq (Get-Content diff_report_20260315_reviewed.html.sha256).Split(' ')[0]) { "OK" } else { "FAILED" }` |
...
```
 ...
 │     → diff_report_YYYYMMDD_reviewed.html（SHA256 ハッシュ埋込）  │
 │     → diff_report_YYYYMMDD_reviewed.html.sha256                 │
 ├─────────────────────────────────────────────────────────────────┤
 │  5. レビュー済み HTML の検証（2つの方法）                         │
 │     A. ブラウザ内: レビュー済み HTML を開き、ヘッダーの            │
 │        「Verify integrity」ボタンをクリック → ファイルを選択       │
 │        → 合格/不合格ダイアログが表示される                        │
 │     B. コマンドライン: コンパニオン .sha256 ファイルを使用          │
 │        （下記の OS 別コマンドを参照）                              │
 └─────────────────────────────────────────────────────────────────┘
```

レビュー済み HTML と `.sha256` ファイルを一緒に提出することで、改ざんのない監査記録となります。SHA256 ハッシュはレビュー済み HTML 自体にも埋め込まれているため、「Verify integrity」ボタンでダウンロード後にファイルが変更されていないことを独立して確認できます。
