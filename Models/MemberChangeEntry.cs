namespace FolderDiffIL4DotNet.Models
{
    ...
    /// <param name="MemberType">For Field/Property: the declared type using fully qualified .NET names (e.g. "System.String", "System.Int32"). Empty for Method/Constructor/Type entries. For Modified entries, shows "old → new" when type changed (e.g. "System.String → System.Int32"). / フィールド・プロパティの宣言型（完全修飾 .NET 名を使用）。メソッド・コンストラクタ・Type の場合は空。Modified エントリでは型変更時に「旧 → 新」形式で表示。</param>
    /// <param name="ReturnType">For Method: the return type using fully qualified .NET names (e.g. "System.Void", "System.String"). For Constructor: "System.Void". Empty for Type/Field/Property entries. / メソッドの戻り値型（完全修飾 .NET 名を使用）。コンストラクタは "System.Void"。Type/Field/Property の場合は空。</param>
    /// <param name="Parameters">For Method/Constructor: the parameter list without parentheses, using fully qualified .NET type names (e.g. "System.Int32 page", "System.String name, System.Int32 count = 0"). Empty string for no-arg methods. Empty for Type/Field/Property entries. / メソッド・コンストラクタのパラメータ一覧（括弧なし、完全修飾 .NET 型名を使用）。引数なしは空文字列。Type/Field/Property の場合は空。</param>
