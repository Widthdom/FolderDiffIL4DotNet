...
If `status --json` reports `fold_ready=false`, run `cdidx backfill-fold` to upgrade folded-name metadata without reparsing source files. Use `cdidx . --rebuild` only when a full rebuild is actually needed.
