...
- Use `cdidx status --json` to verify freshness (`indexed_at`, `latest_modified`, `git_head`, `git_is_dirty`) when results look suspicious.
- Prefer `cdidx inspect "SymbolName"` first when you want a one-round-trip symbol summary with declaration, body, related symbols, call graph data, and workspace freshness metadata.
- Use `cdidx definition "SymbolName" --body` when you need the declaration and implementation body of a specific symbol.
- Use `cdidx references`, `cdidx callers`, and `cdidx callees` for supported languages before falling back to raw-text search.
...
cdidx excerpt Services/FileDiffService.cs:220-280              # targeted source excerpt
...
- 検索結果が怪しいときは `cdidx status --json` で `indexed_at`, `latest_modified`, `git_head`, `git_is_dirty` を確認して鮮度を判断すること。
- 1 回の問い合わせで宣言・本体・関連シンボル・コールグラフ・ワークスペース鮮度まで見たいときは、最初に `cdidx inspect "SymbolName"` を優先すること。
...
cdidx excerpt Services/FileDiffService.cs:220-280              # ピンポイント抜粋
