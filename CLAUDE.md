...
4. **DEVELOPER_GUIDE.md** — Does this change affect architecture, DI registration, execution flow, or file structure tables?
...
7. **Cross-cutting consistency table** — Consult the [checklist table](#cross-cutting-consistency-rule) above for all related artifacts that need updating in the same commit.
...
3. **README.md** — ユーザー向けの動作、CLI オプション、デフォルト値、使用例に影響するか？英語・日本語の両セクションを更新。
4. **DEVELOPER_GUIDE.md** — アーキテクチャ、DI 登録、実行フロー、ファイル構成表に影響するか？
5. **TESTING_GUIDE.md** — 新しいテストクラスを追加したか？範囲マップ表とテスト件数を更新（EN+JA）。
6. **CLAUDE.md** — アーキテクチャ、設計判断、開発ルールに影響するか？
7. **横断的整合性チェックリスト表** — 上記の[チェックリスト表](#横断的整合性ルール)を参照し、同一コミットで更新すべき関連成果物をすべて確認。
8. **PR 説明** — このコミットで PR のスコープが変わったか？タイトル・説明を最終状態に合わせて更新。
