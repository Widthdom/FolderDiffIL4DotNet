...
- `Services/HtmlReport/js/` (12 JS module files, concatenated with comment stripping at generation time)
...
- **`FolderDiffIL4DotNet.Plugin.Abstractions` was first published to NuGet from commit `588fa933df8730c3f77f5a939b4eb030ee17041c`.** If any changes are made to files under `FolderDiffIL4DotNet.Plugin.Abstractions/` that advance the Plugin.Abstractions library beyond this commit, **you MUST immediately notify the user** so they can decide whether to publish a new NuGet package version. This applies to any functional change — new APIs, bug fixes, breaking changes, or dependency updates in Plugin.Abstractions.
...
- `Services/HtmlReport/diff_report.css`
- `Services/HtmlReport/js/`（12個のJSモジュールファイル、生成時に結合・コメント除去）
...
- **`FolderDiffIL4DotNet.Plugin.Abstractions` は初回コミット `588fa933df8730c3f77f5a939b4eb030ee17041c` で NuGet に公開済み。** `FolderDiffIL4DotNet.Plugin.Abstractions/` 配下のファイルに、このコミット以降の進化となる変更（新 API、バグ修正、破壊的変更、依存関係更新など）を加えた場合は、**必ずユーザーに即座に通知すること。** 新しい NuGet パッケージバージョンの公開が必要かどうかの判断はユーザーが行う。
