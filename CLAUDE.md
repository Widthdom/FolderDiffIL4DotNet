...
- `Services/HtmlReport/js/` (12 JS module files, concatenated and minified at generation time)
- `Services/HtmlReport/JsMinifier.cs` (NUglify wrapper for JS/CSS minification)

These are declared in `FolderDiffIL4DotNet.csproj` as `<EmbeddedResource>` and loaded at runtime by `HtmlReportGenerateService`. JS and CSS are minified via NUglify with the result cached per-process. When modifying HTML report behavior, edit the source files directly — not C# string literals.
...
- `Services/HtmlReport/diff_report.css`
- `Services/HtmlReport/js/`（12個のJSモジュールファイル、生成時に結合・ミニファイ）
- `Services/HtmlReport/JsMinifier.cs`（JS/CSS ミニファイ用 NUglify ラッパー）

これらは `FolderDiffIL4DotNet.csproj` で `<EmbeddedResource>` として宣言され、`HtmlReportGenerateService` が実行時にロードする。JS と CSS は NUglify でミニファイされ、結果はプロセスごとにキャッシュされる。HTML レポートの動作を変更する場合は、C# 文字列リテラルではなくソースファイルを直接編集すること。
