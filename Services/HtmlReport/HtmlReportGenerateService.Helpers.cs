...
namespace FolderDiffIL4DotNet.Services
{
    ...
    public sealed partial class HtmlReportGenerateService
    {
            ...
            string importanceLevels = "")
        {
            ...
            // Status column — marker and background color based on section / Status列 — セクションに基づくマーカーと背景色
            ...
            string statusCls = sectionPrefix switch
            {
                "add" => " col-status-add",
                "rem" => " col-status-rem",
                "mod" or "sha256w" or "tsw" => " col-status-mod",
                _ => ""
            };
            sb.AppendLine($"  <td class=\"col-status{statusCls}\">{HtmlEncode(statusMarker)}</td>");
