...
namespace FolderDiffIL4DotNet.Services
{
    ...
    public sealed partial class DotNetDisassembleService : IDotNetDisassembleService
    {
            ...
            bool recordUsage)
        {
            ...
            {
                _logger.LogMessage(AppLogLevel.Warning, $"Failed to get IL from cache for assembly '{dotNetAssemblyFileAbsolutePath}' with command '{disassembleCommand}' ({ex.GetType().Name}): {ex.Message}", shouldOutputMessageToConsole: true, ex);
                return (false, null, null);
            }
        }
