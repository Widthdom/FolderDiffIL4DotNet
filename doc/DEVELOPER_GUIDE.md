...
The HTML report includes a client-side filter zone that allows users to narrow down file rows by multiple criteria. The filter zone is preserved in reviewed HTML so reviewers can also filter, and its filter/legend content can be collapsed as a single panel.
...
- `toggleFilterZone()` / `initFilterZoneToggle()` — hide or restore the full filter/legend panel and keep `aria-expanded` aligned for normal and reviewed HTML.
...
2. The filter zone is **outside** `<!--CTRL-->...<!--/CTRL-->` markers so that reviewed HTML retains full filter functionality. Filter checkboxes, the search input, and the collapse toggle remain interactive in reviewed mode.
...
HTML レポートには、複数の条件でファイル行を絞り込めるクライアントサイドフィルターゾーンが含まれています。フィルターゾーンは reviewed HTML にも引き継がれ、レビュアーもフィルタ可能です。フィルター/凡例コンテンツは1つのパネルとして折りたためます。
...
- `toggleFilterZone()` / `initFilterZoneToggle()` — フィルター/凡例パネル全体を非表示または復元し、通常 HTML と reviewed HTML の `aria-expanded` を同期する。
...
1. フィルタ状態は `collectState()` と reviewed state 埋め込みペイロードから意図的に除外。ライブレポートでは `{storageKey}-filters` に別保存する。
2. フィルターゾーンは `<!--CTRL-->` マーカーの**外**に配置し、reviewed HTML でもフィルタ機能を完全に維持。フィルタチェックボックス、検索入力、折りたたみトグルは reviewed モードでも操作可能。
