...
- `toggleFilterZone()` / `initFilterZoneToggle()` — hide or restore the full filter/legend panel, persist live-report collapsed state under `{storageKey}-filter-zone-collapsed`, and keep `aria-expanded` aligned. Reviewed HTML ignores localStorage and restores the baked DOM state.
...
- `downloadReviewed()` — clears all `filter-hidden` / `filter-hidden-parent` classes and inline table widths before capturing `outerHTML`, preserves the current filter-zone collapsed DOM state in the reviewed file, then restores live page state via `syncTableWidths()`.
...
6. `downloadReviewed()` must clear inline `style="width:..."` from tables before `outerHTML` capture. Otherwise, stale pixel widths from `syncTableWidths()` are baked into the reviewed HTML and cause column width mismatch on reviewed load.
...
- `toggleFilterZone()` / `initFilterZoneToggle()` — フィルター/凡例パネル全体を非表示または復元し、通常レポートの折りたたみ状態を `{storageKey}-filter-zone-collapsed` に保存して `aria-expanded` を同期する。reviewed HTML は localStorage を無視し、焼き込まれた DOM 状態を復元する。
- `wrapInputWithClear(inp)` — 検索入力をクリアボタン（⊗）付きラッパーで囲む。`input` と `change` の両イベントを発火。
- `downloadReviewed()` — `filter-hidden` / `filter-hidden-parent` クラスとテーブルの inline width をクリアしてから `outerHTML` をキャプチャし、現在のフィルターゾーン折りたたみ DOM 状態を reviewed ファイルに保持してから、`syncTableWidths()` でライブページの状態を復元。
...
3. フィルターゾーンは `<!--CTRL-->` マーカーの**外**に配置し、reviewed HTML でもフィルタ機能を完全に維持。フィルタチェックボックス、検索入力、折りたたみトグルは reviewed モードでも操作可能。reviewed HTML はブラウザローカルの通常レポート用パネル状態を無視し、`downloadReviewed()` がキャプチャした DOM 状態から開始する。
...
6. `downloadReviewed()` は `outerHTML` キャプチャ前にテーブルの inline `style="width:..."` をクリアすること。そうしないと `syncTableWidths()` が設定した古いピクセル幅が reviewed HTML に焼き込まれ、reviewed ロード時に列幅の不一致が発生する。
