...
| [`Services/HtmlReportGenerateService.cs`](../Services/HtmlReportGenerateService.cs) | Interactive HTML review report generation | Reads [`FileDiffResultLists`](../Models/FileDiffResultLists.cs) only; produces a self-contained [`diff_report.html`](samples/diff_report.html) with checkboxes, text inputs, localStorage auto-save, and download function; "Download as reviewed" computes SHA256 of the reviewed HTML via Web Crypto API and downloads a companion `.sha256` verification file; skipped when [`ShouldGenerateHtmlReport`](../Models/ConfigSettings.cs) is `false` |
...
| [`Services/HtmlReportGenerateService.cs`](../Services/HtmlReportGenerateService.cs) | インタラクティブ HTML レビューレポート生成 | [`FileDiffResultLists`](../Models/FileDiffResultLists.cs) を読むだけ；チェックボックス・テキスト入力・localStorage 自動保存・ダウンロード機能を持つ自己完結型 [`diff_report.html`](samples/diff_report.html) を生成；「Download as reviewed」は Web Crypto API でレビュー済み HTML の SHA256 を計算し、コンパニオン `.sha256` 検証ファイルもダウンロード；[`ShouldGenerateHtmlReport`](../Models/ConfigSettings.cs) が `false` のときはスキップ |
