...
Current tree has `582` total tests in the latest full run (`dotnet test FolderDiffIL4DotNet.Tests/FolderDiffIL4DotNet.Tests.csproj -p:UseAppHost=false --nologo`): `581` passing, `1` skipped (E2E test requiring a real disassembler binary).
...
直近のフル実行（`dotnet test FolderDiffIL4DotNet.Tests/FolderDiffIL4DotNet.Tests.csproj -p:UseAppHost=false --nologo`）の合計は `582` 件です（成功 `581`、スキップ `1` ― 実際の逆アセンブラバイナリが必要な E2E テスト）。
