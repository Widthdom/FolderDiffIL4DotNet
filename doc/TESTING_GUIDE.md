...
| HTML レポート JavaScript | [`diff_report.test.js`](../JsTests/diff_report.test.js)（Jest/jsdom、40 テスト） | `formatTs` 日付フォーマット、`collectState` チェックボックス/テキスト/テキストエリア収集とフィルタ ID 除外、`autoSave` localStorage 永続化とステータス表示、`applyFilters` 重要度/未チェックのみ/検索フィルタリングと関連 diff-row の非表示、`resetFilters` デフォルト状態復元、`decodeDiffHtml` base64-UTF8 デコード（マルチバイト文字含む）、`collapseAll` details 要素の折りたたみ、`clearAll` 入力リセットと confirm ガード、DOMContentLoaded 時の `__savedState__` および localStorage からの状態復元、レビュー済みモードの読み取り専用化、`verifyIntegrity` null ガードアラート、`setupLazyDiff` トグル時の遅延デコード/挿入と重複デコード防止、`updateProgress` バー幅/テキスト/完了クラス/localStorage統合/詳細内訳、破損 localStorage 耐性（不正 JSON、空文字列、null 値、非オブジェクト JSON、localStorage 容量超過、存在しない要素 ID、型不一致値） |
