...
The latest full run (`dotnet test FolderDiffIL4DotNet.Tests/FolderDiffIL4DotNet.Tests.csproj -p:UseAppHost=false --nologo`) includes `1` skipped test (an E2E test that requires a real disassembler binary).
...
直近のフル実行（`dotnet test FolderDiffIL4DotNet.Tests/FolderDiffIL4DotNet.Tests.csproj -p:UseAppHost=false --nologo`）では、実際の逆アセンブラバイナリが必要な E2E テストが `1` 件スキップされます。
