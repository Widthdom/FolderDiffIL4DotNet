...
- If inline diff rendering is skipped because the source paths are malformed, the warning now records the compared old/new roots or IL report paths so path-construction mistakes can be isolated without re-running under a debugger.
- If an optional `checklist.json` exists but cannot be read, the warning now also records the active LocalApplicationData override metadata so it is easier to tell whether the problem came from the default app-data location or a custom override.
- If the NuGet vulnerability pass fails in best-effort mode, the warning now includes a short package preview (`Packages=[...]`) so you can see which dependency batch was being checked without enabling extra logging.
...
### Inline IL diff shows `0 vs N lines`

**Symptom:** `diff_report.html` shows a message such as `Inline diff skipped: edit distance too large ... in 0 vs N lines` for an `ILMismatch`.

**Cause:** The HTML inline diff reads generated IL text from `Reports/IL/old/*.txt` and `Reports/IL/new/*.txt`. A `0` old or new count means that side's generated IL text file was empty at report-generation time. A stale 0-byte IL cache file, or a warning such as `Failed to write IL cache file` that left an old cache file unchanged, can cause this.

**Solution:** Check the log around the same file for warnings such as `Ignored empty IL cache ...`, `IL comparison raw disassembly produced an empty line set`, or `Inline diff IL source contains an empty side`. Empty IL cache entries are removed and recomputed; if you need to bypass cache state for a run, use:

```bash
nildiff /path/old /path/new label --no-il-cache --no-pause
```
...
Recent `Disk quota trim:` info logs also include whether trimming was triggered by file count, total bytes, or both, plus the pre-trim counts. Use that to decide whether to lower `ILCacheMaxDiskFileCount`, `ILCacheMaxDiskMegabytes`, or both.
...
- source path 不正で inline diff 描画がスキップされた場合、warning には比較対象 old/new ルートまたは IL レポートパスも含まれるため、デバッガを付け直さなくてもパス組み立てミスを切り分けやすくなります。
- 任意の `checklist.json` が存在するのに読み込めない場合、warning には有効中の LocalApplicationData override 情報も含まれるため、既定 app-data 由来かカスタム override 由来かを切り分けやすくなります。
- NuGet 脆弱性チェックがベストエフォートで失敗した場合、warning には短い package preview（`Packages=[...]`）も含まれるため、追加ログを有効にしなくてもどの依存関係バッチを見ていたか把握しやすくなります。
...
### Inline IL diff に `0 vs N lines` と出る

**症状:** `ILMismatch` で `diff_report.html` に `Inline diff skipped: edit distance too large ... in 0 vs N lines` のようなメッセージが出る。

**原因:** HTML inline diff は `Reports/IL/old/*.txt` と `Reports/IL/new/*.txt` の生成済み IL テキストを読みます。old/new のどちらかが `0` 行なら、レポート生成時点でその側の IL テキストファイルが空だったという意味です。古い 0 バイト IL cache ファイルや、`Failed to write IL cache file` warning により既存 cache ファイルが更新されず残った状況で起こりえます。

**解決策:** 同じファイル付近のログで `Ignored empty IL cache ...`、`IL comparison raw disassembly produced an empty line set`、`Inline diff IL source contains an empty side` などの warning を確認してください。空の IL cache エントリは削除され再生成されます。実行時に cache 状態を切り離したい場合は次を使います:
...
最近の `Disk quota trim:` info ログには、件数超過・総バイト超過・両方のどれで trim が始まったかと、trim 前の件数も含まれます。`ILCacheMaxDiskFileCount` と `ILCacheMaxDiskMegabytes` のどちらを下げるべきか判断材料にしてください。
