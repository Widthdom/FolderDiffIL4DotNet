...
Subsequence (LCS)** using dynamic programming. You build an $N \times M$ table
(where $N$ = lines in old, $M$ = lines in new) and fill every cell.

| Metric | Classic LCS     | Myers Diff       |
| ------ | --------------- | ---------------- |
| Time   | $O(N \times M)$ | $O(D^2 + N + M)$ |
| Space  | $O(N \times M)$ | $O(D^2)$         |
...
Every point $(x, y)$ represents a state: "we have consumed $x$ lines from old
and $y$ lines from new."
...
| ---------- | ----------------------- | ---------------------------- | ------------- |
| Horizontal | $(x, y) \to (x+1, y)$   | Delete `old[x]`              | 1 edit        |
| Vertical   | $(x, y) \to (x, y+1)$   | Insert `new[y]`              | 1 edit        |
| Diagonal   | $(x, y) \to (x+1, y+1)$ | Match — `old[x]` == `new[y]` | **0** (free!) |
...
> **With exactly** **$D$** **edits, only diagonals** **$k \in \{-D, -D+2, \ldots, D-2, D\}$** **are
> reachable.**

Why? Each edit changes $k$ by exactly $\pm 1$ (horizontal: $k+1$, vertical:
$k-1$). Starting at $k = 0$, after $D$ moves the parity of $k$ always equals
the parity of $D$. So for $D = 0$, only $k = 0$ is reachable; for $D = 1$,
$k \in \{-1, +1\}$; for $D = 2$, $k \in \{-2, 0, +2\}$; and so on.
...
2. **Snake extensions**: Each cell $(x, y)$ in the edit graph is visited _at
   ...
   snakes can only advance forward, so the total number of string comparisons
   is bounded by **$N + M$**.
...
The V array itself is $O(D)$. But we save a snapshot at each of the $D + 1$
steps. Snapshot at step $d$ has size $2d + 3$. Total:

```math
\sum_{d=0}^{D}(2d + 3) \approx D^2 + 3D \approx D^2 \text{ integers}
```
...
| Completely different files   | $N + M$ | Degrades to $O((N+M)^2)$  | Up to $N+M$ snapshots   |
...
```math
\sum_{d=0}^{4000}(2d + 3) \approx 16 \text{ million integers} \approx 64 \text{ MB}
```
...
| -------------------- | ------------------------ | --------------- | ------------------------ |
| Classic LCS (DP)     | $O(N \times M)$          | $O(N \times M)$ | Small files only         |
| Myers (basic)        | $O(D^2 + N + M)$         | $O(D^2)$        | Files with few changes ✓ |
| Myers (linear space) | $O(D^2 + N + M)$         | $O(D)$          | Memory-constrained       |
| Patience diff        | $O(N \log N + D^2)$      | $O(N)$          | Code with unique lines   |
| Histogram diff       | $\approx O(N + M + D^2)$ | $O(N + M)$      | Git's default for code   |
...
ことです。old の行数 **$N$**、new の行数 **$M$** として **$N \times M$** のテーブルを作り、全セルを
埋めます。

| 指標 | 古典的 LCS         | Myers Diff       |
| -- | --------------- | ---------------- |
| 時間 | **$O(N \times M)$** | **$O(D^2 + N + M)$** |
| 空間 | **$O(N \times M)$** | **$O(D^2)$**         |
...
| 移動   | 方向                      | 意味                        | コスト        |
| ---- | ----------------------- | ------------------------- | ---------- |
| 横移動  | **$(x, y) \to (x+1, y)$**   | `old[x]` を削除              | 1 回の編集     |
| 縦移動  | **$(x, y) \to (x, y+1)$**   | `new[y]` を挿入              | 1 回の編集     |
| 斜め移動 | **$(x, y) \to (x+1, y+1)$** | 一致 — `old[x]` == `new[y]` | **0**（無料！） |
...
> **$D$** **回の編集では、対角線** **$k \in \{-D, -D+2, \ldots, D-2, D\}$** **にのみ到達可能。**

なぜか？ 各編集は **$k$** をちょうど **$\pm 1$** 変化させます（横移動: **$k+1$**、縦移動: **$k-1$**）。
**$k = 0$** からスタートし、**$D$** 回移動後の **$k$** のパリティ（偶奇）は常に **$D$** のパリティと
一致します。つまり **$D = 0$** なら **$k = 0$** のみ、**$D = 1$** なら **$k \in \{-1, +1\}$**、
**$D = 2$** なら **$k \in \{-2, 0, +2\}$**…と展開します。
...
```math
\sum_{d=0}^{D}(2d + 3) \approx D^2 + 3D \approx D^2 \text{ 個の整数}
```
...
| 完全に異なるファイル        | **$N + M$** | **$O((N+M)^2)$** に退化      | **$N+M$** 個のスナップショット    |
...
```math
\sum_{d=0}^{4000}(2d + 3) \approx 1600 \text{ 万整数} \approx 64 \text{ MB}
```
...
| アルゴリズム         | 時間                       | 空間              | 最適な用途           |
| -------------- | ------------------------ | --------------- | --------------- |
| 古典的 LCS (DP)   | **$O(N \times M)$**          | **$O(N \times M)$** | 小さなファイルのみ       |
| Myers（基本版）     | **$O(D^2 + N + M)$**         | **$O(D^2)$**        | 差分が少ないファイル ✓    |
| Myers（線形空間版）   | **$O(D^2 + N + M)$**         | **$O(D)$**          | メモリ制約がある場合      |
| Patience diff  | **$O(N \log N + D^2)$**      | **$O(N)$**          | ユニークな行が多いコード    |
| Histogram diff | **$\approx O(N + M + D^2)$** | **$O(N + M)$**      | Git のコード差分デフォルト |
