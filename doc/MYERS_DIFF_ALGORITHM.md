...
> [日本語版](#日本語版--myers-diff-アルゴリズム詳解)) provides the same content
...
- [日本語版 — Myers Diff アルゴリズム詳解](#日本語版--myers-diff-アルゴリズム詳解)
...
For two 2,370,000-line IL files that differ in only 20 lines:

- **LCS**: 2,370,000 × 2,370,000 = **5.6 trillion** cells — completely
  infeasible.
- **Myers**: D = 20 → roughly 400 diagonal iterations + 4.74 million snake
...
|---|---|---|---|
| 2.37M-line IL files, 20 changes | 20 | ~95M ops (< 0.1 s) | ~400 ints (negligible) |
...
- **Original paper**: E. W. Myers, ["An O(ND) Difference Algorithm and Its
  Variations"](http://www.xmailserver.org/diff2.pdf), 1986.
...
最短差分を求める教科書的な方法は、**最長共通部分列 (LCS)** を動的計画法で解く
...
20 行しか違わない 237 万行の IL ファイル 2 本を比較する場合:

- **LCS**: 2,370,000 × 2,370,000 = **5.6 兆**セル — 実行不可能。
- **Myers**: D = 20 → 対角線反復 約 400 回 + スネーク比較 474 万回 →
  **0.1 秒未満**で完了。
...
| シナリオ | D | 時間 | トレース領域 |
|----------|---|------|-------------|
| 237 万行 IL、差分 20 行 | 20 | ~9500 万演算（< 0.1 秒） | ~400 整数（無視できる） |
