...
| `ILCacheDirectoryAbsolutePath` | (empty) | Cache directory path. Empty disables disk cache. |
...
| --- | --- | --- | --- |
| IL Memory Cache | 50-200 MB | 50-200 MB (capped at 1,000 entries) | Yes (entry count) |
...
**Key insight:** The IL memory cache is the largest memory consumer. With the default 1,000-entry cap and typical IL text sizes (10-200 KB per assembly), peak memory for the cache alone is approximately 100-200 MB. For projects with 10,000+ assemblies, the cache serves as a sliding window over the most recently accessed entries.
...
| `ILCacheDirectoryAbsolutePath` | （空） | キャッシュディレクトリパス。空の場合はディスクキャッシュ無効。 |
...
| --- | --- | --- | --- |
| IL メモリキャッシュ | 50-200 MB | 50-200 MB（1,000 エントリ上限） | はい（エントリ数） |
...
**重要な洞察：** IL メモリキャッシュが最大のメモリ消費者。デフォルトの 1,000 エントリ上限と典型的な IL テキストサイズ（アセンブリあたり 10-200 KB）では、キャッシュ単体のピークメモリは約 100-200 MB。10,000+ アセンブリのプロジェクトでは、キャッシュは最近アクセスされたエントリのスライディングウィンドウとして機能。
